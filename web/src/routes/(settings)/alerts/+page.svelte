<script lang="ts">
	...
	import { cat2names, formatBytes, formatBytesPerSecond, types2names, url, var2unit } from '$lib/utils';
	...
	onMount(async () => {
		...
		alertVars = await fetch(
			import.meta.env.PROD ? url('api/alert_vars') : 'http://localhost:30000/api/alert_vars'
		)
			.then((response) => {
				if (!response.ok) {
					console.error('Failed to fetch alert vars:', response.status);
				}
				return response.json();
			})
			.catch((error) => {
				console.error('Error fetching alert vars:', error);
			});

		notifMethods = await fetch(
				...
				{#if notifMethods.length > 0}
					...
					>
                        ...
                        <div class="form-group">
							...
							<input type="number" id="name" min=1 bind:value={alertForm.time_window} required />
						</div>
						...
						<div class="form-group">
							...
							<input type="number" step="0.1" id="threshold" bind:value={alertForm.threshold} required />
						</div>
